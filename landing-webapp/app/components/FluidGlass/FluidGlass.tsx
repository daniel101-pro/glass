...
export default function FluidGlass({
  ...
  return (
    <Canvas camera={{ position: [0, 0, 20], fov: 15 }} gl={{ alpha: true }}
      onCreated={({ gl }) => {
        // Debug: set clear color to visible tint to confirm canvas rendering
        gl.setClearColor(0x00ffff, 0.05);
      }}
    >
...
const ModeWrapper = memo(function ModeWrapper({
  ...
  useFrame((state, delta) => {
    ...
    if (debugRef.current) {
      debugRef.current.position.copy(ref.current.position);
      debugRef.current.position.z = 15.05;
    }
  ...
  return (
    <>
      ...
      {/* DEBUG: visible cursor follower to confirm rendering */}
      <mesh ref={debugRef}>
      ...
      {/* High-contrast visual aid: subtle white outline plane slightly larger to ensure visibility */}
      <mesh position={[0,0,14.9]} scale={[vp.width, vp.height, 1]} renderOrder={1000}>
        <planeGeometry />
